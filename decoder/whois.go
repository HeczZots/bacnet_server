...
func (d *Decoder) decode(data interface{}) {
	...
	d.err = binary.Read(d.buff, btypes.EncodingEndian, data)
}
...
func (d *Decoder) NPDU(n *btypes.NPDU) (addr []btypes.Address, err error) {
	...
	if meta.IsNetworkLayerMessage() {
		...
		if n.NetworkLayerMessageType == btypes.NetworkIs {
			n.Source = &btypes.Address{}
			d.decode(&n.Source.Net)
		}
		if n.NetworkLayerMessageType == btypes.IamRouterToNetwork {
			n.Source = &btypes.Address{}
...
const ServiceUnconfirmedWhoIs btypes.ServiceUnconfirmed = 8
